...
- `pnpm test` -- Run the full node test lane (vitest, currently 2,234 tests across 131 files)
- `pnpm run typecheck` -- Type check without emit
- `pnpm run bench` -- Run benchmarks (tinybench)
- `pnpm run coverage:merge` -- Merge node + browser coverage and enforce repo thresholds
- `pnpm run bench:gate` -- Run replicated benchmark gate and emit artifact data
...
- `docs/adr/0002-zero-alloc.md` -- Zero-allocation hot path discipline (pool, dirty, dense ECS, microtask batching)
- `docs/adr/0004-plan-coordinator.md` -- Plan IR vs RuntimeCoordinator (per-tick phase sequencing)
...
- `pnpm exec vitest run --config vitest.config.ts tests/unit/compositor.test.ts` to run single file
