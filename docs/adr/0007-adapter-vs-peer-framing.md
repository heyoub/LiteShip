...
The repo compiler's harness template for `siteAdapter` emits round-trip tests (native → LiteShip projection → native equivalence) and a host-capability matrix so adapter bugs surface in the gauntlet.
