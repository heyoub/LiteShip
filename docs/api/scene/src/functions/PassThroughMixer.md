[**czap**](../../../README.md)

***

[czap](../../../README.md) / [scene/src](../README.md) / PassThroughMixer

# Function: PassThroughMixer()

> **PassThroughMixer**(`frameIndex`, `sink`): `SystemShape`

Defined in: [scene/src/systems/pass-through-mixer.ts:22](https://github.com/TheFreeBatteryFactory/czap/blob/main/packages/scene/src/systems/pass-through-mixer.ts#L22)

Build a PassThroughMixer keyed to a frame index + receipt sink.

## Parameters

### frameIndex

`number`

### sink

(`receipt`) => `void`

## Returns

`SystemShape`
